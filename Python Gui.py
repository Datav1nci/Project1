...
class FinancialsTab(QWidget):
    ...
    def init_ui(self):
        ...
        headers = ['Date', 'Description', 'Sub-Description', 'Type', 'Amount', 'Category', 'ROWID']
        ...
        trans_tab = QWidget()
        t1_layout = QVBoxLayout(trans_tab)
        t1_layout.addWidget(self.table)
        self.tabs.addTab(trans_tab, "Transactions")

        # Summary Tab
        summary_tab = QWidget()
        t2_layout = QVBoxLayout(summary_tab)

        self.summary_table = QTableWidget()
        self.summary_table.setColumnCount(2)
        self.summary_table.setHorizontalHeaderLabels(['Category', 'Total Amount'])
        self.summary_table.setSortingEnabled(True)
        # Prevent last column auto-stretch, bring totals right next to category
        hdr = self.summary_table.horizontalHeader()
        hdr.setStretchLastSection(False)
        hdr.setSectionResizeMode(0, QHeaderView.Stretch)
        hdr.setSectionResizeMode(1, QHeaderView.ResizeToContents)
        t2_layout.addWidget(self.summary_table)

        # Pie chart canvas
        self.figure = Figure(figsize=(4, 4))
        self.pie_canvas = FigureCanvas(self.figure)
        t2_layout.addWidget(self.pie_canvas)

        self.tabs.addTab(summary_tab, "By Category")
...
""", [start, end])
            ...
            QMessageBox.critical(self, 'Error', f'Failed to load summary: {e}')
            return

        self.summary_table.setRowCount(len(rows))
        for r, (cat, total) in enumerate(rows):
            self.summary_table.setItem(r, 0, QTableWidgetItem(str(cat)))
            amt_item = QTableWidgetItem(f"{total:.2f}")
            amt_item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
            self.summary_table.setItem(r, 1, amt_item)

        # redraw pie chart
        self.plot_pie()

    def plot_pie(self):
        # gather data, only positive totals
        labels, sizes = [], []
        for i in range(self.summary_table.rowCount()):
            total = float(self.summary_table.item(i, 1).text())
            if total > 0:
                labels.append(self.summary_table.item(i, 0).text())
                sizes.append(total)
